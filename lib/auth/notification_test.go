...
func TestNotifications(t *testing.T) {
	...
	require.Equal(t, expectedUserNotifsNextKey, resp.UserNotificationsNextPageToken)
	require.Equal(t, expectedGlobalNotifsNextKey, resp.GlobalNotificationsNextPageToken)
	...
	resp, err = auditorClient.ListNotifications(ctx, &notificationsv1.ListNotificationsRequest{
		PageSize:                     10,
		UserNotificationsPageToken:   resp.UserNotificationsNextPageToken,
		GlobalNotificationsPageToken: resp.GlobalNotificationsNextPageToken,
	})
	require.NoError(t, err)
	finalOut = append(finalOut, resp.Notifications...)
	// Verify that all the notifications are in the list and in correct order, and that the nextKeys are empty.
	require.Equal(t, auditorExpectedNotifications, notificationsToTitlesList(t, finalOut))
	require.Equal(t, "", resp.UserNotificationsNextPageToken)
	require.Equal(t, "", resp.GlobalNotificationsNextPageToken)
