...
// trustedClusterCATypes is the set of root cluster CA types that are
// trusted by leaf clusters.
var trustedClusterCATypes = []types.CertAuthType{
	types.HostCA,
	types.UserCA,
	types.DatabaseCA,
	types.OpenSSHCA,
...
func (a *Server) DeleteTrustedCluster(ctx context.Context, name string) error {
	...
	for _, caType := range trustedClusterCATypes {
...
func (a *Server) DeleteRemoteCluster(ctx context.Context, clusterName string) error {
	...
	// to err on the side of paranoia and delete the full set of trusted cluster CAs.
	var ids []types.CertAuthID
	for _, caType := range trustedClusterCATypes {
		ids = append(ids, types.CertAuthID{
			Type:       caType,
